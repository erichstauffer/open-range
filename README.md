...
`assets/robot.png` and `assets/robot-vector.svg` are source art the robot sprite
was drawn from by hand — reference material for whoever edits it next, never
shipped to the browser and never loaded at runtime.
...
slots for eyes, the panelled chest. The file in `assets/` is a reference for
